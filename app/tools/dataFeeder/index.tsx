...
export default function DataFeeder({ agents, selectedAgent, setSelectedAgent }: any) {
  ...
  const handleCancel = async () => {
    try {
      ...
      setError("Failed to cancel data feeding");
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between">
          ...
          >
            <div className="h-min">
              {selectedLayerCount > 0
                ...
                : "Select some layers with text to replace"}
            </div>
          </div>
        </div>
            ...
            <span>
              {selectedAgent
                ? (<>
                  ...
                  <span className="font-semibold">UXWritingPal</span> are recommended to create realistic data.
                </>)
              : "Select an Agent"}
            </span>
          </div>
      ...
      <div className="flex flex-col gap-4 relative">
        ...
        {/* Success Pill */}
        <SuccessPill show={showSuccess} message="Replacement complete" />
